            ...
            <div className="container mx-auto px-6 py-4">
                <div className="flex items-center justify-between gap-6">
                    ...
                    <div className="hidden lg:flex items-center gap-6">
                            ...
                            <div className="relative w-16 h-16">
                                ...
                                <svg className="transform -rotate-90 w-16 h-16">
                                    ...
                                    <circle
                                        ...
                                        cy="32"
                                        r="28"
                                        className={`bg-gradient-to-r ${getConfidenceColor(data.confidence_score)}`}
                                        stroke="url(#confidenceGradient)"
                                        strokeWidth="6"
                                        fill="none"
                                        strokeDasharray={`${(data.confidence_score / 100) * 175.93} 175.93`}
                                        strokeLinecap="round"
                                    />
                                    <defs>
                                        <linearGradient id="confidenceGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                                            <stop offset="0%" className={data.confidence_score >= 80 ? "stop-emerald-500" : data.confidence_score >= 60 ? "stop-yellow-500" : "stop-orange-500"} />
                                            <stop offset="100%" className={data.confidence_score >= 80 ? "stop-green-600" : data.confidence_score >= 60 ? "stop-amber-600" : "stop-red-600"} />
                                        </linearGradient>
                                    </defs>
                                </svg>
                                <div className="absolute inset-0 flex items-center justify-center">
                                    <span className="text-lg font-bold text-white">{data.confidence_score}</span>
                                </div>
                            </div>
                            <div>
                                ...
                                <div className="text-sm font-medium text-gray-400">
                                    {data.confidence_score >= 80 ? 'High' : data.confidence_score >= 60 ? 'Medium' : 'Low'} Signal
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-[#18181b] border border-[#27272a]">
                            <Database className={`w-5 h-5 ${getResearchDepthColor(data.research_depth)}`} />
                            <div>
                                <div className="text-xs font-bold uppercase tracking-widest text-gray-500">
                                    Research Depth
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="w-24 h-2 bg-[#27272a] rounded-full overflow-hidden">
                                        <div
                                            className={`h-full bg-gradient-to-r ${getConfidenceColor(data.research_depth)} transition-all duration-500`}
                                            style={{ width: `${data.research_depth}%` }}
                                        />
                                    </div>
                ...
                <div className="lg:hidden mt-4 grid grid-cols-2 gap-3">
                    <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[#18181b] border border-[#27272a]">
                        <TrendingUp className={`w-4 h-4 ${getConfidenceColor(data.confidence_score).includes('emerald') ? 'text-emerald-500' : 'text-yellow-500'}`} />
                        <div>
                            <div className="text-xs text-gray-500">Confidence</div>
                            <div className="text-sm font-bold text-white">{data.confidence_score}%</div>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[#18181b] border border-[#27272a]">
                        <Database className={`w-4 h-4 ${getResearchDepthColor(data.research_depth)}`} />
                        <div>
                            <div className="text-xs text-gray-500">Research</div>
                            <div className="text-sm font-bold text-white">{data.research_depth}%</div>
                        </div>
                    </div>
                </div>
            </div>
