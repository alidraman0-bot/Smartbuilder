...
export default function LoginPage() {
    ...
    const handleOAuth = async (provider: 'google' | 'github') => {
        ...
        // If no error, the user is redirected to the provider
    };
        ...
        return (
            <div className="min-h-screen bg-[#0a0a0f] flex items-center justify-center p-6 overflow-hidden relative">
                ...
                >
                    ...
                    <h2 className="text-2xl font-semibold text-white mb-2">Loading your workspace...</h2>
                    <p className="text-[#8a8a9a]">Redirecting to your production environment</p>
                </motion.div>
            </div>
        );
    }
                ...
                <div className="w-full max-w-[400px] pt-24 pb-12">
                    ...
                    >
                        ...
                        <div className="w-full mb-8">
                            ...
                            <p className="text-[#8a8a9a] text-sm font-medium opacity-80">Access your production environment.</p>
                        </div>

                        <div className="w-full space-y-5">
                            {/* Social Auth at the top like reference */}
                            <div className="grid grid-cols-2 gap-4">
                                <SocialButton type="google" onClick={() => handleOAuth('google')} />
                                <SocialButton type="github" onClick={() => handleOAuth('github')} />
                            </div>
                            ...
                            <form onSubmit={handleLogin} className="space-y-4">
                                <div>
                                    <label className="block text-[11px] font-bold text-[#5a5a6a] uppercase tracking-[0.1em] mb-2" htmlFor="email">
                                        Email
                                    </label>
                                    <input
                                        id="email"
                                        ...
                                        required
                                        className="w-full bg-white/5 border border-white/5 rounded-xl py-3 px-4 text-sm text-white placeholder:text-[#3a3a4a] outline-none focus:ring-1 focus:ring-indigo-500/30 focus:border-indigo-500/50 transition-all font-medium"
                                    />
                                </div>

                                <div>
                                    <div className="flex items-center justify-between mb-2">
                                        <label className="block text-[11px] font-bold text-[#5a5a6a] uppercase tracking-[0.1em]" htmlFor="password">
                                            Password
                                        </label>
                                        <Link href="#" className="text-[11px] font-bold text-indigo-400/60 hover:text-indigo-400 transition-colors uppercase tracking-wider">
                                            Forgot?
                                        </Link>
                                    </div>
                                ...
                                <button
                                    type="submit"
                                    disabled={isLoading}
                                    className="w-full relative group disabled:opacity-70 disabled:grayscale disabled:cursor-not-allowed mt-3"
                                >
                                    <div className="absolute -inset-0.5 bg-gradient-to-r from-indigo-500 to-purple-500 rounded-xl blur opacity-30 group-hover:opacity-50 transition duration-300" />
                                    <div className="relative w-full bg-indigo-600 hover:bg-indigo-500 text-white py-3 rounded-xl text-sm font-bold flex items-center justify-center space-x-2 transition-all">
                                        {isLoading ? (
                                            <>
                            ...
                            <div className="text-center pt-2">
                                <p className="text-[13px] font-medium text-[#8a8a9a]">
                                    Don't have a Smartbuilder account?{' '}
                                    <Link href="/signup" className="text-indigo-400 hover:text-indigo-300 transition-all font-bold">
                                        Sign up
                                    </Link>
                                </p>
                            </div>

                            <div className="mt-10 flex items-center justify-center space-x-2.5 text-[#3a3a4a]">
...
function SocialButton({ type, onClick }: { type: 'google' | 'github', onClick?: () => void }) {
    const isGoogle = type === 'google';
    return (
        <button type="button" onClick={onClick} className="flex items-center justify-center space-x-3 px-4 py-3 bg-[#0f0f15] border border-white/5 rounded-xl hover:bg-[#14141a] hover:border-white/10 transition-all group">
            {isGoogle ? (
                <svg className="w-5 h-5" viewBox="0 0 24 24">
                    ...
                    <path fill="#4285F4" d="M23.64 12.2c0-.85-.07-1.68-.2-2.48H12v4.7h6.53c-.28 1.5-1.14 2.79-2.42 3.65l3.91 3.03c2.29-2.11 3.62-5.21 3.62-8.91z" />
            ...
            <span className="text-sm font-semibold text-[#b4b4c4] lg:group-hover:text-white transition-colors capitalize">{type}</span>
        </button>
    );
}
